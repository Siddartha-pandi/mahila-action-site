...
export async function GET(req: NextRequest) {
  ...
  try {
    const result = await queryDb(
      "SELECT id, name, email, phone, role, kind, skills, status, created_at FROM users ORDER BY created_at DESC"
    );
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const { name, email, phone, role, kind, status, password, skills } = body || {};
    ...
    await queryDb(
      `INSERT INTO users (id, name, email, phone, role, kind, password_hash, skills, status) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)`,
      [id, name.trim(), normalizedEmail, normalizedPhone, assignedRole, kind || null, password_hash, skills || null, assignedStatus]
    );
